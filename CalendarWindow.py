...
class CalendarWindow:
    ...
    def show(self, language, item_list, already_selected_items, chart_name):
        ...
        ttk.Button(self.button_frame, text=utils.setLabel(language, 'Anuluj', 'Cancel'), command=self.top_window.destroy).place(relx=0.55, rely=0.16,
                                                                                                                                     relwidth=0.4, relheight=0.70)
        ...
        self.start_date_btn = ttk.Button(self.calendar_frame, text=self.start_date, command=lambda: self.showCalendarWindow(self.root.winfo_x() + self.root.winfo_width() / 2, self.root.winfo_y() + self.root.winfo_height() / 2, 310, 390, dateutil.parser.parse(self.start_date), language, mode='start'))
        self.end_date_btn = ttk.Button(self.calendar_frame, text=self.end_date, command=lambda: self.showCalendarWindow(self.root.winfo_x() + self.root.winfo_width() / 2, self.root.winfo_y() + self.root.winfo_height() / 2, 310, 390, dateutil.parser.parse(self.end_date), language, mode='end'))
        self.start_date_btn.place(relx=0.22, rely=0.11, relwidth=0.75, relheight=0.34)
        self.end_date_btn.place(relx=0.22, rely=0.55, relwidth=0.75, relheight=0.34)
    ...
    def updateStartEndDates(self):
        ...
        if self.start_date == 'None':
            self.start_date = self.item_list[self.selected_items[0]]
        if self.end_date == 'None':
            self.end_date = self.item_list[self.selected_items[1]]

    def showCalendarWindow(self, x, y, width, height, current_date, language, mode):
        self.showLoadingCursor(self.top_window)
        posx = x - width / 2
        posy = y - height / 2
        self.calendar_window = tkinter.Toplevel(self.top_window)
        self.calendar_window.geometry('%dx%d+%d+%d' % (width, height, posx, posy))
        ...
        self.calendar_window.protocol('WM_DELETE_WINDOW', self.exitCalendarWindow)        
        ...
        ttk.Button(button_frame, text='OK', command=lambda: self.overwriteDate(mode)).place(relx=0.05, rely=0.16, relwidth=0.4, relheight=0.7)
        ttk.Button(button_frame, text=utils.setLabel(language, 'Anuluj', 'Cancel'), command=self.exitCalendarWindow).place(relx=0.55, rely=0.16, relwidth=0.4, relheight=0.7)

        self.cal = tkcalendar.Calendar(date_frame, selectmode='day', year=current_date.year, month=current_date.month, day=current_date.day)
        self.cal.place(relx=0.02, rely=0.02, relwidth=0.96, relheight=0.96)

        self.hour_var = tkinter.StringVar(value=utils.addZero(str(current_date.hour)))
        self.minute_var = tkinter.StringVar(value=utils.addZero(str(current_date.minute)))
        self.second_var = tkinter.StringVar(value=utils.addZero(str(current_date.second)))

        hour_combobox = ttk.Combobox(time_frame, textvariable=self.hour_var, values=[utils.addZero(str(i)) for i in range(24)], state='readonly')
        minute_combobox = ttk.Combobox(time_frame, textvariable=self.minute_var, values=[utils.addZero(str(i)) for i in range(60)], state='readonly')
        second_combobox = ttk.Combobox(time_frame, textvariable=self.second_var, values=[utils.addZero(str(i)) for i in range(60)], state='readonly')
        ...
        Hovertip(second_combobox, utils.setLabel(language, 'Sekunda', 'Second'))
        hour_combobox.place(relx=0.16, rely=0.16, relwidth=0.2, relheight=0.7)
        ttk.Label(time_frame, text=':').place(relx=0.37, rely=0.16, relwidth=0.02, relheight=0.7)
        minute_combobox.place(relx=0.4, rely=0.16, relwidth=0.2, relheight=0.7)
        ttk.Label(time_frame, text=':').place(relx=0.61, rely=0.16, relwidth=0.02, relheight=0.7)
        second_combobox.place(relx=0.64, rely=0.16, relwidth=0.2, relheight=0.7)

        self.hideLoadingCursor(self.top_window)       

    def overwriteDate(self, mode):
        self.showLoadingCursor(self.calendar_window)
        year = str(dateutil.parser.parse(self.cal.get_date(), dayfirst=True).year)
        month = str(dateutil.parser.parse(self.cal.get_date(), dayfirst=True).month)
        day = str(dateutil.parser.parse(self.cal.get_date(), dayfirst=True).day)
        hour = self.hour_var.get()
        minute = self.minute_var.get()
        second = self.second_var.get()
        
        date_str = year + '-' + utils.addZero(month) + '-' + utils.addZero(day) + ' ' + utils.addZero(hour) + ':' + utils.addZero(minute) + ':' + utils.addZero(second)
        date = dateutil.parser.parse(date_str)
        date_list = [dateutil.parser.parse(item) for item in self.item_list]
        ...
        if mode == 'start':
            ...
            date_in_list = self.item_list[-1]
            for dl in date_list:
                if dl >= date:
                    date_in_list = str(dl)
                    break
            self.selected_items[0] = self.item_list.index(date_in_list)
        elif mode == 'end':
            if self.chart_name == 'A':
            ...
            date_in_list = self.item_list[0]
            date_list.reverse()
            for dl in date_list:
                if dl <= date:
                    date_in_list = str(dl)
                    break
            self.selected_items[1] = self.item_list.index(date_in_list)
