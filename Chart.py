...
class Chart:
    def __init__(self, root, frame, refresh_button_frame, excel_table, language):
        ...
        self.text_window = TextWindow(root, 300, 450)
    ...
    def switchFilters(self, chart_name, chart_frame, filters_btn):
            ...
            if show_filters:
                ...
                filters_btn['text'] = utils.setLabel(self.language, u'Filtry \u25b2', u'Filters \u25b2')
            else:
    ...
    def drawChartA(self, chart_drawn=None):
        ...
        ttk.Button(options_frame, text=utils.setLabel(self.language, 'Numer seryjny', 'Serial number'),
                   command=lambda: self.checklist_window.show(self.language, self.serial_number, self.A_serial_number_selected, page_len=250)).pack(side=tkinter.LEFT, padx=15)
        ttk.Button(options_frame, text=utils.setLabel(self.language, 'Stacja', 'Station'),
                   command=lambda: self.checklist_window.show(self.language, self.station, self.A_station_selected, page_len=250)).pack(side=tkinter.LEFT)
        ttk.Button(options_frame, text=utils.setLabel(self.language, 'Operator', 'Operator name'),
                   command=lambda: self.checklist_window.show(self.language, self.operator_name, self.A_operator_name_selected, page_len=250)).pack(side=tkinter.LEFT, padx=15)
        ttk.Button(options_frame, text=utils.setLabel(self.language, 'Data', 'Date'),
                   command=lambda: self.checklist_window.show(self.language, self.date, self.A_date_selected, page_len=250)).pack(side=tkinter.LEFT)
    ...
    def drawChartB(self, chart_drawn=None):
        ...
        if self.chartB_first_draw:
            ...
            self.B_date_selected = self.onesList(len(self.date))
            ...
            for sn in self.serial_number:
                engine_station = self.getColumn('Stacja', 'Station', 1)[(self.getColumn('Numer seryjny', 'Serial Number', 10) == sn)].values
        ...
        if not self.chartB_drawn:
            ...
            self.B_first_station = self.station[self.B_station_selected[0]]
            self.B_last_station = self.station[self.B_station_selected[1]]
            self.B_stations = self.station[self.B_station_selected[0]:self.B_station_selected[1] + 1]

            self.B_engines_total = len(self.serial_number)
            self.B_invalid_engines = self.B_wrong_transition_count
            self.B_valid_engines = self.B_engines_total - self.B_invalid_engines
            print(self.B_station_nmb)
            print(self.B_first_station)
            print(self.B_last_station)
            print(self.B_stations)
            print(self.B_engines_total)
            print(self.B_invalid_engines)
            print(self.B_valid_engines)
            ###############
        ...
        ttk.Separator(options_frame, orient='horizontal').place(
            relx=0, rely=0.98, relwidth=1)
        ttk.Separator(filters_frame, orient='horizontal').place(relx=0, rely=0.98, relwidth=1)

        if self.chartB_show_filters:
            filters_btn_text = utils.setLabel(self.language, u'Filtry \u25b2', u'Filters \u25b2')
        else:
            filters_btn_text = utils.setLabel(self.language, u'Filtry \u25bc', u'Filters \u25bc')
        filters_btn = ttk.Button(options_frame, text=filters_btn_text, command=lambda: self.switchFilters('B', chart_frame, filters_btn))
        filters_btn.pack(side=tkinter.LEFT, padx=15)
        ttk.Button(options_frame, text=utils.setLabel(self.language, 'Szczegóły', 'Details'), command=lambda: self.text_window.show({})).pack(side=tkinter.LEFT)
        self.show_cdf_var = tkinter.IntVar(value=1 if self.show_cdf else 0)
        ttk.Checkbutton(options_frame, text=utils.setLabel(self.language, 'Pokaż dystrybuantę', 'Show CDF'), variable=self.show_cdf_var, command=lambda: self.switchShowCDFVar('B')).pack(side=tkinter.LEFT, padx=15)

        ttk.Button(filters_frame, text=utils.setLabel(self.language, 'Stacja', 'Station'),
                   command=lambda: self.combobox_window.show(self.language, self.station, self.B_station_selected)).pack(side=tkinter.LEFT, padx=15)
        ttk.Button(filters_frame, text=utils.setLabel(self.language, 'Data', 'Date'),
                   command=lambda: self.checklist_window.show(self.language, self.date, self.B_date_selected, page_len=250)).pack(side=tkinter.LEFT)
        ttk.Button(filters_frame, text=utils.setLabel(self.language, 'Zmiana', 'Shift'),
                   command=lambda: self.checklist_window.show(self.language, self.shift, self.B_shift_selected, page_len=250)).pack(side=tkinter.LEFT, padx=15)
        ttk.Button(filters_frame, text=utils.setLabel(self.language, 'Cykl', 'Cycle'),
                   command=lambda: self.checklist_window.show(self.language, self.cycle, self.B_cycle_selected, page_len=250)).pack(side=tkinter.LEFT)
        ttk.Button(filters_frame, text=utils.setLabel(self.language, 'Numer części', 'Part number'),
                   command=lambda: self.checklist_window.show(self.language, self.part_number, self.B_part_number_selected, page_len=250)).pack(side=tkinter.LEFT, padx=15)
        ...
        ax1.legend(lengend_handles, legend_labels, bbox_to_anchor=(1, 0.92 + 0.02 * len(lengend_handles)))
