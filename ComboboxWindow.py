...
class ComboboxWindow:
    ...
    def show(self, language, item_list, already_selected_items):
        ...
        self.combobox_window.geometry('%dx%d+%d+%d' % (self.width, self.height, self.posx, self.posy))
        ...
        self.combobox_frame.place(x=0, y=0, width=self.width, height=self.height - 40)
        self.combobox_line_frame1.place(relx=0, rely=0, relwidth=1, relheight=0.5)
        self.combobox_line_frame2.place(relx=0, rely=0.5, relwidth=1, relheight=0.5)
        ...
        self.ok_button = ttk.Button(self.button_frame, text='OK', command=self.overwriteRange)
        self.cancel_button = ttk.Button(self.button_frame, text=utils.setLabel(language, 'Anuluj', 'Cancel'), command=self.destroy)
        ...
        self.label1 = ttk.Label(self.combobox_line_frame1, text=utils.setLabel(language, 'Od: ', 'From: '))
        self.label2 = ttk.Label(self.combobox_line_frame2, text=utils.setLabel(language, 'Do: ', 'To:   '))
        self.var1 = tkinter.StringVar(value=self.item_list[self.selected_items[0]])
        self.var2 = tkinter.StringVar(value=self.item_list[self.selected_items[1]])
        self.combobox1 = ttk.Combobox(self.combobox_line_frame1, textvariable=self.var1, values=self.item_list, state='readonly')
        self.combobox2 = ttk.Combobox(self.combobox_line_frame2, textvariable=self.var2, values=self.item_list, state='readonly')
        self.combobox1.bind('<<ComboboxSelected>>', lambda event: self.selectItem1(language))
        self.combobox2.bind('<<ComboboxSelected>>', lambda event: self.selectItem2(language))
    ...
    def selectItem1(self, language):
        ...
        if self.selected_items[0] != selected_item_idx: 
            if self.selected_items[1] >= selected_item_idx:
                ...
                tkinter.messagebox.showerror(message=utils.setLabel(language, 'Wartość początkowa nie może być większa od końcowej.', 'Initial value cannot be greater than the final one.'))
    ...
    def selectItem2(self, language):
        ...
        if self.selected_items[1] != selected_item_idx: 
            if self.selected_items[0] <= selected_item_idx:
                ...
                tkinter.messagebox.showerror(message=utils.setLabel(language, 'Wartość końcowa nie może być mniejsza od początkowej.', 'Final value cannot be lesser than initial one.'))
